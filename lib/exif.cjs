...
async function gifToWebp(media) {
    ...
    const tmpFileIn = isPath ? media : path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.gif`);
...
async function imageToWebp(media) {
    const isPath = typeof media === 'string';
    const tmpFileIn = isPath ? media : path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.jpg`);
    const tmpFileOut = path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`);
    try {
        if (!isPath) await fs.promises.writeFile(tmpFileIn, media);
        await new Promise((resolve, reject) => {
...
async function videoToWebp(media) {
    const isPath = typeof media === 'string';
    const tmpFileIn = isPath ? media : path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.mp4`);
    const tmpFileOut = path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`);
    try {
        if (!isPath) await fs.promises.writeFile(tmpFileIn, media);
        await new Promise((resolve, reject) => {
...
async function writeExif(media, data) {
    const isPath = typeof media === 'string';
    let tmpFileIn = isPath ? media : path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.tmp`);
    const tmpFileOut = path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`);
    try {
        if (!isPath) await fs.promises.writeFile(tmpFileIn, media);
